...
import math
from .submodule import convbn_3d, feature_extraction, DisparityRegression, RecoverySize
...
class PSMNet(nn.Module):
    def __init__(self, in_channels, start_disp, maxdisp, struct_fea_c, fuse_mode,
        ...
        self.classif3 = nn.Sequential(convbn_3d(32, 32, 3, 1, 1),
                                      ...
                                      nn.Conv3d(32, 1, kernel_size=3, padding=1, stride=1, bias=False))
        if refine == 'csr':
            self.refine_module = DeformableRefineF(in_channels, feature_c=64, node_n=2, modulation=True, cost=True)
        else:
            self.refine_module = DeformableRefineF(in_channels, feature_c=64, node_n=2, modulation=True, cost=False)
    ...
    def forward(self, left, right):
        ...
        if self.refine == 'csr':
            ...
            predr = DisparityRegression(self.start_disp, self.maxdisp, win_size=win_s)(distributer)
