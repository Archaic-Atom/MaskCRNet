...
from .raft_position_encoding import PositionEncodingSine
from .raft_transformer import LocalFeatureTransformer
...
class CREStereo(nn.Module):
    def __init__(self, in_channles, max_disp=192, mixed_precision=False, test_mode=False):
        ...
        self.context_dim = self.in_channles // 2
        self.dropout = 0
        ...
        self.self_att_fn = LocalFeatureTransformer(
            d_model=self.in_channles, nhead=8, layer_names=["self"] * 1, attention="linear"
        )
        self.cross_att_fn = LocalFeatureTransformer(
            d_model=self.in_channles, nhead=8, layer_names=["cross"] * 1, attention="linear"
        )

        # adaptive search
        self.search_num = 9
        self.conv_offset_16 = nn.Conv2d(
            self.in_channles, self.search_num * 2, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, image1, image2, flow_init=None, iters=20, upsample=True, test_mode=False):
        ...
        with autocast(enabled=self.mixed_precision):
            ...
            pos_encoding_fn_small = PositionEncodingSine(
                d_model=self.in_channles, max_shape=(image1.shape[2] // 4, image1.shape[3] // 4)
            )
            # 'n c h w -> n (h w) c'
            ...
            fmap1_dw16, fmap2_dw16 = [
                x.reshape(x.shape[0], image1.shape[2] // 4, -1, x.shape[2]).permute(0, 3, 1, 2)
