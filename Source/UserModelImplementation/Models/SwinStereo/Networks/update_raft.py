...
class BasicMotionEncoder(nn.Module):
    def __init__(self, cor_planes: int) -> None:
        super().__init__()
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, hidden_dim: int, cor_planes: torch.Tensor, mask_size: int = 8):
        super().__init__()

        self.encoder = BasicMotionEncoder(cor_planes)
        self.gru = SepConvGRU(hidden_dim=hidden_dim, input_dim=128 + hidden_dim)
        self.flow_head = FlowHead(hidden_dim, hidden_dim=256)
        self.mask = nn.Sequential(nn.Conv2d(hidden_dim, 256, 3, padding=1),
                                  nn.ReLU(inplace=True),
                                  nn.Conv2d(256, mask_size**2 * 9, 1, padding=0))

    def forward(self, net: torch.Tensor, inp: torch.Tensor,
                corr: torch.Tensor, flow: torch.Tensor):
